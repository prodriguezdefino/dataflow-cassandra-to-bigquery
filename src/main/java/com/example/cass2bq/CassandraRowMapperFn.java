...
/**
 * This is an implementation of the Cassandra Mapper interface which allows custom Row Mapping logic to be injected into the Beam
 * CassandraIO. The goal of this implementation is to take a Cassandra Resultset and for each of the rows map them into a BeamRow.
 *
...
class CassandraRowMapperFn implements Mapper<Row>, Serializable {
  ...
  /**
   * This method creates a Beam {@link Schema schema} based on the list of Definition supplied as input. If a field name matches a key from
   * the keyOrder map (supplied as a input parameter) this denotes that the field was previously part of the Cassandra primary-key. To
   * preserve the order of these primary keys metadata is added to the field on the format "key_order": X - where X is an integer indicating
   * the order of the key in Cassandra.
   *
   * @param definitions a list of field definitions.
   * @param keyOrder the order of the fields that make up the Cassandra row key. key is field name, value denotes the order from the
   * row-key.
  ...
  /**
   * This method takes Objects retrieved from Cassandra as well as their Column type and converts them into Objects that are supported by
   * the Beam {@link Schema} and {@link Row}.
   *
   * <p>
   * The method starts with the complex types {@link List}, {@link Map} and {@link Set}. For all other types it assumes that they are
   * primitives and passes them on.
   *
  ...
  /**
   * This method converts Cassandra {@link DataType} to Beam {@link FieldType}. Tuples are as of yet not supported as there is no
   * corresponding type in the Beam {@link Schema}.
   *
   ...
   * @link <a href="https://docs.datastax.com/en/cql/3.3/cql/cql_reference/tupleType.html">Cassandra Tuple</a>
   */
  ...
  /**
   * Most primitivies are represented the same way in Beam and Cassandra however there are a few that differ. This method converts the
   * native representation of timestamps, uuids, varint, dates and IPs to a format which works for the Beam Schema.
   *
   * <p>
   * Dates and Timestamps are returned as DateTime objects whilst UUIDs are converted to Strings. Varint is converted into BigDecimal. The
   * rest simply pass through as they are.
   *
