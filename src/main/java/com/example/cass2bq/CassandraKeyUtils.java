...
/**
 * This helper class allows fetching a sorted map containing the columns that make up the (compound) primary key in a @see
 * <a href="http://cassandra.apache.org/">Apache Cassandra</a> table. See method documentation for futher details.
 */
class CassandraKeyUtils {
  ...
  /**
   * This method return a CQL statement to fetch the columns in the primary key with in the table and corresponding keyspace supplied as
   * input. If position>-1 the column is part of the primary key.
   *
   * <p>
   * SELECT column_name, kind, position FROM system_schema.columns WHERE keyspace_name='mykeyspace' AND table_name='mytable' AND position>-1
   * ALLOW FILTERING;
   */
  ...
  /**
   * This method returns a sorted map containing the columns that make up the (compound) primary key in cassandra.
   *
   * <p>
   * Each key in return value contains a name of a column in the cassandra key. The value for each corresponding key in the map denotes the
   * order in the Cassandra key.
   *
   * <p>
   * Example, given the table:
   *
   * <p>
   * create table mytable ( key_part_one int, key_part_two int, data boolean, PRIMARY KEY(key_part_one, key_part_two) );
   *
   * <p>
   * The method would return the following map:
   *
   * <p>
   * "key_part_one", 0 "key_part_two", 1
   *
